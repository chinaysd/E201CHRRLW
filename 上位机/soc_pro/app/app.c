...
unsigned char HEAT_COOL_ARR[3]={0,0,0};
...
void App_LockKeyHandle(void){
	if(App_Para.LockFlag){		
		...
		if(App_Para.HC_Sta == STOP_MODE){
			...
			APP_SET_BACKLIGHT(TP_KEY0_LED);    //Tp_LedSta &=  0xef     1110 1111
            LED5_PIN = ON;
		}
        #if 0
		...
		switch(App_Para.Chg_GetKey){	
			...
			case CHG_KEY1_VALUE:{
				if(App_Para.Chg_CurKey != CHG_KEY1_VALUE){					
					...
					//App_Key4Handle(); //推杆B打开
					App_Key1Handle();//推杆B收回
				}
				break;
			}
